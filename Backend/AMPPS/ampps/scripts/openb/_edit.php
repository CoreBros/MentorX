...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `id` FROM `".$__settings['dbprefix']."users` WHERE `username` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			$update_query = "UPDATE `".$__settings['dbprefix']."users` SET `password` = '".$__settings['admin_pass']."' WHERE `id` = '".$userid."';";

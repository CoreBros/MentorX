...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `user_id` FROM `".$__settings['dbprefix']."users` WHERE `user_name` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			$update_query = "UPDATE `".$__settings['dbprefix']."users` SET `user_password` = '".$__settings['admin_pass']."', `user_salt` = '".$__settings['salt']."', `user_admin_salt` = '".$__settings['salt']."' WHERE `user_id` = '".$userid."';";

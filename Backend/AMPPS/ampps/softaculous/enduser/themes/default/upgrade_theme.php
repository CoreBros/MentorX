...
function upgrade_theme(){
	...
	<div id="install_win">';
		...
		if(empty($upgraded)){
					...
					<form accept-charset="'.$globals['charset'].'" name="installsoftware" method="post" action="" onsubmit="return checkform();" id="installsoftware">
						...
						<div class="bg"><br />
							...
							if(empty($globals['disable_backup_upgrade'])){
								...
								<div class="row">
									<div class="col-sm-5">
										<label class="sai_head">'.$l['create_backup'].'</label><br />
								...
								<div class="row" id="backup_options" style="display:none;">
									<div class="col-sm-5">
								...
								<div class="row" id="advanced_backup" style="display:none;">
									<div class="col-sm-5"></div>
									<div class="col-sm-7">
										<input type="checkbox" name="backup_dir" id="backup_dir" '.POSTchecked('backup_dir', true).' />&nbsp;&nbsp;<label for="backup_dir" class="sai_head">'.$l['backup_dir'].'</label>
									</div><br />';
									...
									if(!empty($softins['wwwdir'])){
										...
										<div class="col-sm-7">
											<input type="checkbox" name="backup_wwwdir" id="backup_wwwdir" '.POSTchecked('backup_wwwdir', true).' />&nbsp;&nbsp;<label for="backup_wwwdir" class="sai_head">'.$l['backup_wwwdir'].'</label>
										</div><br />';
									}
									...
									if(!empty($softins['softdatadir'])){
										echo '<div class="col-sm-5"></div>
										<div class="col-sm-7">
											<input type="checkbox" name="backup_datadir" id="backup_datadir" '.POSTchecked('backup_datadir', true).' />&nbsp;&nbsp;<label for="backup_datadir" class="sai_head">'.$l['backup_datadir'].'</label>
										</div><br />';
									}
									...
									if(!empty($softins['softdb'])){
										echo '<div class="col-sm-5"></div>
										<div class="col-sm-7">
											<input type="checkbox" name="backup_db" id="backup_db" '.POSTchecked('backup_db', true).' />&nbsp;&nbsp;<label for="backup_db" class="sai_head">'.$l['backup_db'].'</label>
										</div><br />';
									}	
								echo '</div>';
							}
			...
			$return = '<a href="'.script_link($soft).'&highlight='.$softins['insid'].'&postact=upgrade">'.$l['return'].'</a>';
			//Filter to customize the return to link
			$return = apply_filters('return_link', $return, $act);

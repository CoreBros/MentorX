...
echo '<form accept-charset="'.$globals['charset'].'" name="editadomain" method="post" action="" class="form-horizontal">
	...
	<div class="row" id="ftp_pass">
		<div class="col-sm-5">
			<label for="ftp_pass" class="sai_head" id="ftp_pass_head">'.$l['ftp_pass'].'</label><br />
			<span class="sai_exp2">'.$l['ftp_pass_exp'].((empty($globals['no_prefill_pass'])) ? '' : '<br/>'.$l['ftp_pass_opt']).'</span>
		</div>
		<div class="col-sm-7">
			<div class="row">
				<div class="col-sm-11">
					<input type="password" name="ftp_pass" id="ftp_passwd" class="form-control" value="'.POSTval('ftp_pass', (empty($globals['no_prefill_pass']) ? $thisdomain['ftp_pass'] : '')).'" autocomplete="off" /><br />
				</div>
	...
	'<div class="row" id="trauthpass">
		...
		<div class="col-sm-7">
			...
			<input type="radio" name="auth_password" id="auth_password" value="" onchange="check_auth_password();" '.POSTradio('auth_password', '', (!empty($thisdomain['private_key']) ? '' : null)).' />&nbsp;'.$l['auth_method_key'].'<br /><br />
		</div>
	</div>

...
global $theme, $user, $logged_in, $globals, $l, $dmenus, $onload, $newslinks, $feeds, $softpanel, $iscripts, $catwise, $allcatwise, $soft, $classes_categories, $scripts, $apps, $apps_catwise, $user_sitepad, $ins_apps, $file_data, $act;
	...
	if(empty($globals['off_panel_link'])){
		$navbar_top['goto_control_panel']['fullscreen'] = '<td align="center" width="32" class="someclass" title="'.$l['go_cpanel'].'"><a href="'.$softpanel->theme['panel_url'].'">'.((!empty($softpanel->goto_cp_logo) && empty($user['user_defined_color']) && empty($globals['default_hf_text']) && empty($globals['default_hf_bg'])) ? '<img src="'.$theme['images'].$softpanel->goto_cp_logo.'" height="25" width="25" >' : '<i class="fa sai-control_panel fa-2x"></i>').'</a></td>';
		
		$navbar_top['goto_control_panel']['responsive'] = '<li><a href="'.$softpanel->theme['panel_url'].'">'.((!empty($softpanel->goto_cp_logo) && empty($user['user_defined_color']) && empty($globals['default_hf_text']) && empty($globals['default_hf_bg'])) ? '<img src="'.$theme['images'].$softpanel->goto_cp_logo.'" height="25" width="25" >' : '<i class="fa sai-control_panel fa-2x"></i>').'&nbsp;&nbsp;'.$l['go_cpanel'].'</a></li><br />';
	}
	...
	if(!webuzo()){
		...
		//Navigation Logout
		$navbar_top['goto_logout']['fullscreen'] = '<td align="center" width="32" class="someclass" title="'.$l['go_logout'].'"><a href="'.$softpanel->theme['logout'].'"><i class="fa sai-logout fa-1x"></i></a></td>';
		
		$navbar_top['goto_logout']['responsive'] = '<li><a href="'.$softpanel->theme['logout'].'"><i class="fa sai-logout fa-1x"></i>&nbsp;&nbsp;'.$l['go_logout'].'</a></li><br />';
	}
